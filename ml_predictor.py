...
class ImprovedPricePredictor:
    ...
    def __init__(self, use_meta_labeling: bool = True):
        ...
        self.model_weights = {"rf": 0.25, "gb": 0.25, "xgb": 0.25, "et": 0.25}
    ...
    def create_labels(
        self, df: pd.DataFrame, forward_periods: int = 5, threshold: float = 0.02
    ) -> pd.Series:
        """
    ...
    def train(self, df: pd.DataFrame, test_size: float = 0.2) -> Dict[str, float]:
        ...
        try:
            ...
            self.training_metrics["precision"] = precision_score(
                y_test_1d, y_pred_1d, average="weighted", zero_division=0
            )
            self.training_metrics["recall"] = recall_score(
                y_test_1d, y_pred_1d, average="weighted", zero_division=0
            )
            self.training_metrics["f1"] = f1_score(
                y_test_1d, y_pred_1d, average="weighted", zero_division=0
            )
    ...
    def _ensemble_predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        # Weighted average
        weights = [
            self.model_weights.get(k, 0.25) for k in ["rf", "gb", "et", "xgb"][: len(probas)]
        ]
        total = sum(weights)
    ...
    def predict(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not self.is_trained or not SKLEARN_AVAILABLE:
            return {"signal": 0, "probability": 0.5, "confidence": 0.0, "model_ready": False}
    ...
    def predict_multi(self, df: pd.DataFrame, window: int = 10) -> List[Dict[str, Any]]:
        """
...
MLPredictor = ImprovedPricePredictor
