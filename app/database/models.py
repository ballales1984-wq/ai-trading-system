...
class PortfolioSnapshot(Base):
    ...
    __table_args__ = (
        Index("ix_portfolio_timestamp_equity", "timestamp", "total_equity"),
    )
...
def get_engine(database_url: str = "sqlite:///data/trading.db"):
    ...
    """
    # Check if SQLite (development) or PostgreSQL (production)
    is_sqlite = database_url.startswith("sqlite")
    
    if is_sqlite:
        # SQLite configuration - simpler, no pooling
        ...
        # PostgreSQL/MySQL configuration - connection pooling
        return create_engine(
            database_url,
            echo=False,
