...
class BinanceConnector(BrokerConnector):
    ...
    async def place_order(self, order: BrokerOrder) -> BrokerOrder:
        ...
        try:
            ...
            self.logger.info(f"Order placed: ID={order.broker_order_id} status={order.status}")
            return order
    ...
    async def cancel_order(self, order_id: str, symbol: str) -> bool:
        ...
        try:
            result = await self._signed_request(
                "DELETE", "v3/order", {"symbol": symbol, "origClientOrderId": order_id}
            )
    ...
    async def get_order_status(self, order_id: str, symbol: str) -> BrokerOrder:
        ...
        result = await self._signed_request(
            "GET", "v3/order", {"symbol": symbol, "origClientOrderId": order_id}
        )
    ...
    async def get_balance(self) -> List[AccountBalance]:
        ...
        for b in result.get("balances", []):
            ...
            if total > 0:
                balances.append(
                    AccountBalance(asset=b["asset"], free=free, locked=locked, total=total)
                )
...
class BybitConnector(BrokerConnector):
    ...
    async def _signed_request(self, method: str, endpoint: str, params: Dict = None) -> Any:
        ...
        signature = hmac.new(
            self.secret_key.encode("utf-8"), sign_payload.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    async def cancel_order(self, order_id: str, symbol: str) -> bool:
        ...
        try:
            await self._signed_request(
                "POST",
                "v5/order/cancel",
                {"category": "spot", "symbol": symbol, "orderLinkId": order_id},
            )
            return True
        except Exception as e:
            self.logger.error(f"Failed to cancel order {order_id}: {e}")
    ...
    async def get_balance(self) -> List[AccountBalance]:
        ...
        for account in result.get("list", []):
            for coin in account.get("coin", []):
                ...
                if total > 0:
                    balances.append(
                        AccountBalance(asset=coin["coin"], free=free, locked=locked, total=total)
                    )
        return balances
...
class PaperTradingConnector(BrokerConnector):
    ...
    async def get_balance(self) -> List[AccountBalance]:
        return [
            AccountBalance(asset=k, free=v, locked=0.0, total=v) for k, v in self.balance.items()
        ]
...
def create_broker_connector(
    broker: str, api_key: str = "", secret_key: str = "", testnet: bool = True
) -> BrokerConnector:
