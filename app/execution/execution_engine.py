...
from app.execution.broker_connector import BrokerConnector, Order, OrderSide, OrderType, OrderStatus
from app.execution.algo_execution import AlgoConfig, TWAPExecutor, VWAPExecutor
...
class ExecutionEngine:
    ...
    def __init__(
        ...
        Args:
            broker: Primary broker connector
            additional_brokers: List of secondary brokers for SOR
            risk_engine: Risk engine for validation
            retry_config: Retry configuration
        """
        self.broker = broker
        self.all_brokers = [broker] + (additional_brokers or [])
        self.risk_engine = risk_engine
        self.retry_config = retry_config or RetryConfig()

        # Smart Router
        self.sor = SmartOrderRouter(self.all_brokers)
        ...
        if validate_risk and self.risk_engine:
            ...
            if not risk_check.passed:
                ...
                return ExecutionResult(
                    success=False, message=risk_check.reason, error="risk_rejected"
                )

        # Create order - use uppercase for OrderType
        order = Order(
    ...
    async def _execute_single(self, order: Order, attempt: int) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(
                success=False, message=str(e), error="execution_error", attempts=attempt
            )
    ...
    async def execute_twap(
        ...
        return ExecutionResult(
            success=status["progress"] >= 0.99,
            filled_quantity=status["executed_qty"],
            avg_price=status["avg_price"],
            message=f"TWAP completed: {status['progress']:.1%} executed",
        )

    async def execute_vwap(
        self, symbol: str, side: str, quantity: float, duration_minutes: int, num_chunks: int = 10
    ) -> ExecutionResult:
        """Execute order using VWAP algorithm (U-curve profile)."""
        config = AlgoConfig(
            symbol=symbol,
            side=OrderSide.BUY if side.upper() == "BUY" else OrderSide.SELL,
            total_quantity=quantity,
        ...
        executor = VWAPExecutor(self.broker, config)
        await executor.execute()
        ...
        return ExecutionResult(
            ...
            filled_quantity=status["executed_qty"],
            avg_price=status["avg_price"],
            message=f"VWAP completed: {status['progress']:.1%} executed",
        )
