...
try:
    from src.external.cloudflare_radar_client import CloudflareRadarClient
    _radar_client = None  # Lazy initialization
    def _get_radar_client():
        global _radar_client
        if _radar_client is None:
            _radar_client = CloudflareRadarClient()
...
# ============================================================================
# CLOUDFLARE RADAR ENDPOINTS
# ============================================================================


@router.get("/radar/traffic")
async def get_radar_traffic() -> dict:
    """
    Get global internet traffic patterns from Cloudflare Radar.
    """
    client = _get_radar_client()
    if not client:
        ...
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "data": {
                "global_traffic_index": 95,
                "traffic_by_region": {"europe": 35, "north_america": 28, "asia_pacific": 25},
                "note": "Demo mode - configure Cloudflare Radar API for real data"
            }
        }
    
    try:
        data = await client.get_global_traffic_patterns()
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar traffic error: {e}")
        raise HTTPException(status_code=502, detail=str(e))


@router.get("/radar/attacks")
async def get_radar_attacks() -> dict:
    """
    ...
    if not client:
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "data": {
                "attacks_detected": 15420,
                "note": "Demo mode - configure Cloudflare Radar API for real data"
            }
        }
    
    try:
        data = await client.get_attack_traffic()
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar attacks error: {e}")
        raise HTTPException(status_code=502, detail=str(e))


@router.get("/radar/outages")
async def get_radar_outages(country: str = Query(None, description="Country code (ISO 3166-1 alpha-2)")) -> dict:
    """
    Get internet outages data from Cloudflare Radar.
    """
    client = _get_radar_client()
    if not client:
        return {
    ...
    try:
        data = await client.get_internet_outages(country)
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar outages error: {e}")
        raise HTTPException(status_code=502, detail=str(e))


@router.get("/radar/sentiment")
async def get_radar_sentiment() -> dict:
    """
    Get web activity sentiment indicator from Cloudflare Radar.
    This can be used as a proxy for market sentiment.
    """
    client = _get_radar_client()
    if not client:
        return {
    ...
    try:
        data = await client.get_web_sentiment_indicator()
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar sentiment error: {e}")
        raise HTTPException(status_code=502, detail=str(e))


@router.get("/radar/connectivity")
async def get_radar_connectivity() -> dict:
    """
    Get global internet connectivity metrics from Cloudflare Radar.
    """
    client = _get_radar_client()
    if not client:
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "data": {
                "global_connectivity_score": 92,
                "note": "Demo mode - configure Cloudflare Radar API for real data"
            }
        }
    
    try:
        data = await client.get_connectivity_metrics()
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar connectivity error: {e}")
        raise HTTPException(status_code=502, detail=str(e))


@router.get("/radar/dns")
async def get_radar_dns() -> dict:
    """
    Get DNS query insights from Cloudflare Radar.
    """
    client = _get_radar_client()
    if not client:
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "data": {
                "dns_query_volume": 75000000,
                "note": "Demo mode - configure Cloudflare Radar API for real data"
            }
        }
    
    try:
        data = await client.get_dns_insights()
        return {
            "source": "cloudflare_radar",
            "timestamp": datetime.utcnow().isoformat(),
            "data": data,
        ...
        logger.error(f"Cloudflare Radar DNS error: {e}")
        raise HTTPException(status_code=502, detail=str(e))
