...
class CheckoutSessionRequest(BaseModel):
    ...
    price_id: Optional[str] = Field(None, max_length=100)
...
async def create_checkout_session(
    request: CheckoutSessionRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    if not stripe_api_key and not stripe_payment_link:
        raise HTTPException(
            status_code=503, detail="Payment system not configured. Please contact support."
        )
