...
def _compute_simulated_portfolio_summary(use_realtime_prices: bool = True) -> PortfolioSummary:
    ...
    for p in positions:
        ...
        if current_price > 0 and quantity > 0:
            ...
            if side == "SELL":  # SHORT
                unrealized_pnl = -price_delta * quantity
            else:  # LONG
                unrealized_pnl = price_delta * quantity
        else:
    ...
    total_return_pct = (
        ((total_value - initial_balance) / initial_balance * 100) if initial_balance > 0 else 0.0
    )
...
class OptimizePortfolioRequest(BaseModel):
    ...
    method: str = Field(
        default="max_sharpe",
        description="max_sharpe, min_variance, risk_parity, equal_weight, inverse_volatility",
    )
...
def update_balance(
    new_balance: float = Query(
        ..., ge=1000, le=100000000, description="New initial balance for paper trading"
    ),
):
    """
...
def get_portfolio_summary() -> PortfolioSummary:
    ...
    if real_data.get("total_value", 0) > 0 and positions:
        ...
        market_value = sum(
            p.get("market_value", p.get("quantity", 0) * p.get("current_price", 0))
            for p in positions
        )
        unrealized_pnl = sum(p.get("unrealized_pnl", 0) for p in positions)
        realized_pnl = sum(p.get("realized_pnl", 0) for p in positions)
...
def get_dual_portfolio_summary() -> DualPortfolioSummary:
    ...
    return DualPortfolioSummary(real=real_data, simulated=simulated_data)
...
def _compute_performance_from_history(
    ...
    tracker = PortfolioPerformanceTracker(
        initial_capital=float(values[0]), risk_free_rate=risk_free_rate
    )
    ts = timestamps or []
    ...
    if get_demo_mode() and len(portfolio_data.get("positions", [])) > 0:
        ...
        return [
            Position(
                ...
                unrealized_pnl=(
                    (realtime_prices.get(p["symbol"], p["current_price"]) - p["entry_price"])
                    * p["quantity"]
...
async def get_position(position_id: str) -> Position:
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"Position {position_id} not found"
    )
...
async def get_allocation() -> dict:
    ...
    for p in positions:
        ...
        value = (
            ...
            else float(p.get("market_value", 0))
        )
    ...
    if total_value == 0:
        return {"by_asset_class": {"crypto": 0}, "by_sector": {"crypto": 0}, "by_symbol": {}}
