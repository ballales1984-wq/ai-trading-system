...
async def purchase_lifetime(current_user: User = Depends(get_current_user)):
    """
    ...
    jwt_manager.activate_subscription(
        user=current_user, plan=SubscriptionPlan.LIFETIME, trial_days=0
    )
...
async def get_trial_status(current_user: User = Depends(get_current_user)):
    ...
    return TrialStatusResponse(is_active=is_trial, days_remaining=days_remaining, end_date=end_date)
