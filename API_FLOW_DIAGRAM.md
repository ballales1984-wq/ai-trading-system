# 🔄 API Engine Flow Diagram — Complete Visual Map

## Block Diagram: APIs → Database → Engine → Simulations → Output

```
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                            🌐 EXTERNAL API SOURCES                                  ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  1️⃣ MARKET DATA (Core)        2️⃣ SENTIMENT & NEWS         3️⃣ MACRO EVENTS           ║
║  ┌─────────────────────┐     ┌─────────────────────┐     ┌─────────────────────┐    ║
║  │ • Binance API       │     │ • NewsAPI.org        │     │ • Trading Economics │    ║
║  │ • CoinGecko API     │     │ • Benzinga API       │     │ • EconPulse         │    ║
║  │ • Alpha Vantage     │     │ • Twitter/X API      │     │ • Investing.com     │    ║
║  │ • Quandl/Nasdaq     │     │ • GDELT API          │     │                     │    ║
║  └────────┬────────────┘     └────────┬────────────┘     └────────┬────────────┘    ║
║           │                           │                           │                  ║
║  4️⃣ NATURAL EVENTS            5️⃣ GEOPOLITICS               6️⃣ INNOVATION/ENERGY     ║
║  ┌─────────────────────┐     ┌─────────────────────┐     ┌─────────────────────┐    ║
║  │ • Open-Meteo        │     │ • EventKG/Wikidata  │     │ • EIA API           │    ║
║  │ • Climate TRACE     │     │ • GDELT (conflicts) │     │ • Google Patents    │    ║
║  │ • USGS Water Data   │     │                     │     │ • Lens.org          │    ║
║  └────────┬────────────┘     └────────┬────────────┘     └────────┬────────────┘    ║
║           │                           │                           │                  ║
╚═══════════╪═══════════════════════════╪═══════════════════════════╪══════════════════╝
            │                           │                           │
            └───────────────────────────┼───────────────────────────┘
                                        ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                         📥 DATA INGESTION LAYER                                      ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐       ║
║  │  API Registry│───▶│  Normalizer  │───▶│  Validator   │───▶│  Rate Limiter│       ║
║  │  (Factory)   │    │  (Unified    │    │  (Quality    │    │  (Throttle)  │       ║
║  │              │    │   Schema)    │    │   Checks)    │    │              │       ║
║  └──────────────┘    └──────────────┘    └──────────────┘    └──────┬───────┘       ║
║                                                                      │               ║
╚══════════════════════════════════════════════════════════════════════╪═══════════════╝
                                                                       ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                         💾 CENTRAL DATABASE                                          ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐                   ║
║  │  ohlcv_data      │  │  news_sentiment  │  │  market_events   │                   ║
║  │  ─────────────── │  │  ─────────────── │  │  ─────────────── │                   ║
║  │  asset           │  │  timestamp       │  │  timestamp       │                   ║
║  │  timestamp       │  │  title           │  │  event_type      │                   ║
║  │  open/high/low   │  │  sentiment_score │  │  impact (H/M/L)  │                   ║
║  │  close/volume    │  │  source          │  │  region           │                   ║
║  │  source_api      │  │  confidence      │  │  linked_assets   │                   ║
║  └──────────────────┘  └──────────────────┘  └──────────────────┘                   ║
║                                                                                      ║
║  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐                   ║
║  │  natural_events  │  │  innovation_idx  │  │  geopolitical    │                   ║
║  │  ─────────────── │  │  ─────────────── │  │  ─────────────── │                   ║
║  │  timestamp       │  │  timestamp       │  │  timestamp       │                   ║
║  │  event_type      │  │  sector          │  │  event_type      │                   ║
║  │  intensity       │  │  impact_score    │  │  severity         │                   ║
║  │  region          │  │  technology      │  │  affected_assets │                   ║
║  │  affected_assets │  │  source          │  │  source          │                   ║
║  └──────────────────┘  └──────────────────┘  └──────────────────┘                   ║
║                                                                                      ║
║  ┌──────────────────────────────────────────────────────────────────┐                ║
║  │  Redis Cache (Hot Data) ←→ PostgreSQL/SQLite (Cold Data)        │                ║
║  └──────────────────────────────────────────────────────────────────┘                ║
║                                                                                      ║
╚══════════════════════════════════════════════════════════════════════╪═══════════════╝
                                                                       │
                    ┌──────────────────────────────────────────────────┘
                    ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                         ⚙️ ANALYSIS ENGINE                                           ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐                   ║
║  │ Technical        │  │ Sentiment        │  │ Event            │                   ║
║  │ Indicators       │  │ Analyzer         │  │ Processor        │                   ║
║  │ ──────────────── │  │ ──────────────── │  │ ──────────────── │                   ║
║  │ RSI, MACD, MA    │  │ NLP Scoring      │  │ Macro linking    │                   ║
║  │ Bollinger Bands  │  │ Source weighting  │  │ Natural linking  │                   ║
║  │ Support/Resist   │  │ Confidence calc   │  │ Geopolitical     │                   ║
║  │ Pattern detect   │  │ Trend detection   │  │ Impact scoring   │                   ║
║  └────────┬─────────┘  └────────┬─────────┘  └────────┬─────────┘                   ║
║           │                     │                      │                             ║
║           └─────────────────────┼──────────────────────┘                             ║
║                                 ▼                                                    ║
║  ┌──────────────────────────────────────────────────────────────────┐                ║
║  │              Feature Engineering (Multi-Factor)                   │                ║
║  │  • Semantic variables from events                                │                ║
║  │  • Source weight reinforcement learning                          │                ║
║  │  • Cross-correlation matrix                                      │                ║
║  │  • PCA / Dimensionality reduction                                │                ║
║  └──────────────────────────────────────────────────────┬───────────┘                ║
║                                                          │                           ║
╚══════════════════════════════════════════════════════════╪═══════════════════════════╝
                                                           ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                    🎲 MONTE CARLO SIMULATION ENGINE (5 Levels)                       ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌─────────────────────────────────────────────────────────────────────────────┐     ║
║  │                                                                             │     ║
║  │  Level 1: BASE MONTE CARLO                                                 │     ║
║  │  ├── Input: Historical prices + volatility                                  │     ║
║  │  ├── Method: Geometric Brownian Motion (GBM)                                │     ║
║  │  └── Output: N random price paths                                           │     ║
║  │       │                                                                     │     ║
║  │       ▼                                                                     │     ║
║  │  Level 2: CONDITIONAL MONTE CARLO                                           │     ║
║  │  ├── Input: L1 paths + macro events + sentiment scores                      │     ║
║  │  ├── Method: Event-conditioned path adjustment                              │     ║
║  │  └── Output: Scenario-weighted price paths                                  │     ║
║  │       │                                                                     │     ║
║  │       ▼                                                                     │     ║
║  │  Level 3: ADAPTIVE MONTE CARLO                                              │     ║
║  │  ├── Input: L2 paths + past prediction accuracy                             │     ║
║  │  ├── Method: Reinforcement learning path correction                         │     ║
║  │  └── Output: Self-improving simulation paths                                │     ║
║  │       │                                                                     │     ║
║  │       ▼                                                                     │     ║
║  │  Level 4: MULTI-FACTOR MONTE CARLO                                          │     ║
║  │  ├── Input: L3 paths + natural events + energy data + cross-correlations    │     ║
║  │  ├── Method: Multi-dimensional factor model + regime switching              │     ║
║  │  └── Output: Factor-enriched simulation paths                               │     ║
║  │       │                                                                     │     ║
║  │       ▼                                                                     │     ║
║  │  Level 5: SEMANTIC HISTORY MONTE CARLO                                      │     ║
║  │  ├── Input: L4 paths + geopolitical events + innovation data                │     ║
║  │  ├── Method: Historical pattern matching + cause-effect modeling             │     ║
║  │  └── Output: Realistic scenario paths + black swan detection                │     ║
║  │                                                                             │     ║
║  └─────────────────────────────────────────────────────────────────────────────┘     ║
║                                          │                                           ║
╚══════════════════════════════════════════╪═══════════════════════════════════════════╝
                                           ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                         🧠 DECISION ENGINE                                           ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐       ║
║  │   Signal     │───▶│  Portfolio   │───▶│    Risk      │───▶│  Execution   │       ║
║  │  Generator   │    │  Allocator   │    │   Manager    │    │   Router     │       ║
║  │              │    │              │    │              │    │              │       ║
║  │ BUY/SELL/    │    │ Risk Parity  │    │ VaR/CVaR     │    │ Best venue   │       ║
║  │ HOLD +       │    │ Mean-Var     │    │ Position     │    │ Slippage     │       ║
║  │ confidence % │    │ Momentum     │    │ limits       │    │ control      │       ║
║  └──────────────┘    └──────────────┘    └──────────────┘    └──────┬───────┘       ║
║                                                                      │               ║
╚══════════════════════════════════════════════════════════════════════╪═══════════════╝
                                                                       ▼
╔══════════════════════════════════════════════════════════════════════════════════════╗
║                         📤 OUTPUT LAYER                                              ║
╠══════════════════════════════════════════════════════════════════════════════════════╣
║                                                                                      ║
║  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐             ║
║  │  Dashboard   │  │  REST API    │  │  Trade Log   │  │  Alerts      │             ║
║  │  (Real-time) │  │  (External)  │  │  (Audit)     │  │  (Telegram)  │             ║
║  │              │  │              │  │              │  │              │             ║
║  │ Portfolio    │  │ JSON/REST    │  │ All trades   │  │ BUY/SELL     │             ║
║  │ Scenarios    │  │ endpoints    │  │ P&L history  │  │ Risk alerts  │             ║
║  │ Risk metrics │  │ WebSocket    │  │ Performance  │  │ Drawdown     │             ║
║  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘             ║
║                                                                                      ║
║  ┌──────────────────────────────────────────────────────────────────┐                ║
║  │  🔄 FEEDBACK LOOP: Results → Weight Update → Strategy Adjust    │                ║
║  │     Continuous learning: updates source weights & model params   │                ║
║  └──────────────────────────────────────────────────────────────────┘                ║
║                                                                                      ║
╚══════════════════════════════════════════════════════════════════════════════════════╝
```

---

## 🔗 API-to-Engine Level Mapping

| API Category | Engine Level | Monte Carlo Level | Purpose |
|---|---|---|---|
| **Binance / CoinGecko / Alpha Vantage / Quandl** | Technical Indicators | Level 1 (Base) | Price paths, volatility |
| **NewsAPI / Benzinga / Twitter / GDELT** | Sentiment Analyzer | Level 2 (Conditional) | Signal weighting |
| **Trading Economics / EconPulse / Investing.com** | Event Processor | Level 2-3 (Conditional + Adaptive) | Event-conditioned scenarios |
| **Open-Meteo / Climate TRACE / USGS** | Feature Engineering | Level 4 (Multi-Factor) | Commodity correlations |
| **EventKG / Wikidata / GDELT (geopolitics)** | Feature Engineering | Level 5 (Semantic History) | Pattern matching |
| **EIA / Google Patents / Lens.org** | Feature Engineering | Level 5 (Semantic History) | Stress test & future scenarios |

---

## 🔄 Interaction Flow (Numbered Steps)

```
Step 1: API Registry dispatches requests to all configured APIs
         │
Step 2: Raw data normalized into unified schema (timestamp, value, source, type)
         │
Step 3: Validated data stored in Central Database (6 tables)
         │
Step 4: Analysis Engine loads data from DB
         ├── Technical Indicators computed from ohlcv_data
         ├── Sentiment scores computed from news_sentiment
         └── Event impact computed from market_events + natural_events + geopolitical
         │
Step 5: Feature Engineering creates multi-factor feature vectors
         ├── Semantic variables from events
         ├── Source weight reinforcement learning
         └── Cross-correlation matrix
         │
Step 6: Monte Carlo Engine runs 5-level simulation
         ├── L1: Base random walks from price data
         ├── L2: Conditioned by events + sentiment
         ├── L3: Adaptive from past accuracy
         ├── L4: Multi-factor (natural + energy + correlations)
         └── L5: Semantic history (geopolitics + innovation)
         │
Step 7: Decision Engine generates signals
         ├── BUY/SELL/HOLD with probability + confidence
         ├── Portfolio allocation (risk parity / mean-variance)
         └── Risk checks (VaR, CVaR, position limits)
         │
Step 8: Execution Router sends orders to exchanges
         │
Step 9: Output Layer displays results
         ├── Dashboard (real-time portfolio + scenarios)
         ├── REST API (external access)
         ├── Trade Log (audit trail)
         └── Alerts (Telegram/Email)
         │
Step 10: Feedback Loop
         └── Results update source weights + model parameters
             → Continuous learning cycle
```

---

## 📁 Source File Mapping

| Component | Source File | Role |
|---|---|---|
| API Registry | [`src/external/api_registry.py`](src/external/api_registry.py) | Central API factory & dispatcher |
| Market Data APIs | [`src/external/market_data_apis.py`](src/external/market_data_apis.py) | Binance, CoinGecko, Alpha Vantage, Quandl |
| CoinMarketCap Client | [`src/external/coinmarketcap_client.py`](src/external/coinmarketcap_client.py) | CMC Pro API: quotes, global metrics, sentiment proxy |
| Sentiment APIs | [`src/external/sentiment_apis.py`](src/external/sentiment_apis.py) | NewsAPI, Benzinga, Twitter, GDELT |
| Macro Event APIs | [`src/external/macro_event_apis.py`](src/external/macro_event_apis.py) | Trading Economics, EconPulse, Investing.com |
| Natural Event APIs | [`src/external/natural_event_apis.py`](src/external/natural_event_apis.py) | Open-Meteo, Climate TRACE, USGS |
| Innovation APIs | [`src/external/innovation_apis.py`](src/external/innovation_apis.py) | EIA, Google Patents, Lens.org |
| Data Collector | [`data_collector.py`](data_collector.py) | Market data ingestion |
| Sentiment Collector | [`sentiment_news.py`](sentiment_news.py) | News sentiment analysis |
| Technical Analysis | [`technical_analysis.py`](technical_analysis.py) | RSI, MACD, Bollinger, patterns |
| ML Predictor | [`src/ml_predictor.py`](src/ml_predictor.py) | Price prediction models |
| Decision Engine | [`decision_engine.py`](decision_engine.py) | Signal generation |
| Risk Engine | [`src/risk_engine.py`](src/risk_engine.py) | VaR, CVaR, limits |
| Execution | [`src/execution.py`](src/execution.py) | Order routing |
| Portfolio | [`src/portfolio_optimizer.py`](src/portfolio_optimizer.py) | Allocation optimization |
| Core Engine | [`src/core/engine.py`](src/core/engine.py) | Main trading engine |
| Market API Routes | [`app/api/routes/market.py`](app/api/routes/market.py) | REST endpoints incl. CMC global/quote/sentiment/top |

---

*Document generated for AI Trading System v2.0 — 2026-02-19*
