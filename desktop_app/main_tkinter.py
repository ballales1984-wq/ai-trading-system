...
class SetupDialog:
    ...
    def build_ui(self):
        tk.Label(self.dialog, text=f"Benvenuto in {APP_NAME}", font=("Arial", 14, "bold")).pack(
            pady=10
        )
        tk.Label(self.dialog, text="Inserisci le tue API keys", font=("Arial", 10), fg="gray").pack(
            pady=5
        )
        ...
        tk.Button(
            self.dialog, text="Salva", command=self.save, bg="#4CAF50", fg="white", width=15
        ).pack(pady=10)

        tk.Label(
            self.dialog, text="Le chiavi sono salvate solo sul tuo PC", font=("Arial", 8), fg="gray"
        ).pack(pady=5)
...
class TradingApp:
    ...
    def build_ui(self):
        ...
        # Page buttons - use production URL directly
        tk.Button(
            btn_frame,
            text="Dashboard",
        ...
        self.log_text = scrolledtext.ScrolledText(
            main, height=15, font=("Consolas", 9), bg="#1e1e1e", fg="#00ff00"
        )
        self.log_text.pack(fill=tk.BOTH, expand=True)

        tk.Label(self.root, text=f"v{APP_VERSION}", font=("Arial", 8), fg="gray").pack(
    ...
    def open_dashboard(self):
        """Open dashboard - tries local backend first, then production."""
        if self.backend_running:
            webbrowser.open(f"{BACKEND_URL}/dashboard")
        else:
            webbrowser.open(f"{PRODUCTION_URL}/dashboard")

    def open_orders(self):
        """Open orders page."""
        if self.backend_running:
            webbrowser.open(f"{BACKEND_URL}/orders")
        else:
            webbrowser.open(f"{PRODUCTION_URL}/orders")

    def open_news(self):
        """Open news page."""
        if self.backend_running:
            webbrowser.open(f"{BACKEND_URL}/news")
        else:
            webbrowser.open(f"{PRODUCTION_URL}/news")

    def open_market(self):
        """Open market page."""
        if self.backend_running:
            webbrowser.open(f"{BACKEND_URL}/market")
        else:
            webbrowser.open(f"{PRODUCTION_URL}/market")

    def open_payment(self):
        """Open payment/purchase page."""
        webbrowser.open(f"{PRODUCTION_URL}/payment")
