...
class MockPerformanceAnalyzer:
    ...
    def get_metrics(self):
        ...
        return MockPerformanceMetrics(
            ...
            max_drawdown=abs(np.random.uniform(5, 15)),
            max_drawdown_percent=np.random.uniform(5, 15),
...
class AutomatedTestingFramework:
    ...
    def run_walk_forward_backtest(
        ...
        logger.info(f"Walk-forward complete: mean_return={mean_return:.2%}, sharpe={sharpe_mean:.2f}")
    ...
    def run_paper_trading_test(
        ...
        for i in range(num_trades):
            ...
            if side == "BUY":
                ...
                proceeds = trade_value - slippage - fees
            ...
            # Log equity
            equity = current_balance + (quantity * price if side == "BUY" else 0)
            analyzer.log_equity(
                timestamp=datetime.now() - timedelta(days=days-i),
                equity=equity
            )
        ...
        logger.info(f"Paper trading complete: Sharpe={metrics.sharpe_ratio:.2f}, MaxDD={metrics.max_drawdown_percent:.2f}%")
    ...
    def run_small_capital_simulation(
        self, 
        ...
        for day in range(0, days, 2):  # Every 2 days
            ...
            fees = trade_value * 0.0005
            
            if side == "BUY":
                current_balance -= (trade_value + slippage + fees)
        ...
        logger.info(f"Small capital simulation complete: Sharpe={metrics.sharpe_ratio:.2f}, MaxDD={metrics.max_drawdown_percent:.2f}%")
    ...
    def run_black_swan_stress_test(self) -> List[StressTestResult]:
            ...
            if not survived:
                for i in range(len(price_path) - 1, -1, -1):
                    if price_path[i] >= peak_equity:
                        recovery_time = (len(price_path) - i) * (scenario['duration_hours'] / num_points)
                        break
        ...
        logger.info(f"Stress tests complete: {sum(1 for r in results if r.survived)}/{len(results)} survived")
    ...
    def check_hmm_regime_stability(
        ...
        try:
            ...
            transition_frequency = num_transitions / len(regime_history) if len(regime_history) > 0 else 0
    ...
    def check_feature_importance(
        ...
        logger.info(f"Feature Stability: {stable_features}/{n_features} stable ({stability_ratio:.1%})")
    ...
    def select_stable_features(
        self,
        feature_importance_result: Dict
    ) -> List[int]:
        """
    ...
    def validate_metrics(
        ...
        if metrics.max_drawdown_percent > (self.config.target_max_dd * 100):
            issues.append(f"Max DD {metrics.max_drawdown_percent:.2f}% > {self.config.target_max_dd*100}%")
        
        # Check Win Rate
        if metrics.win_rate < (self.config.target_win_rate_min * 100):
            issues.append(f"Win rate {metrics.win_rate:.1f}% < {self.config.target_win_rate_min*100}%")
        if metrics.win_rate > (self.config.target_win_rate_max * 100):
            issues.append(f"Win rate {metrics.win_rate:.1f}% > {self.config.target_win_rate_max*100}%")
        
        # Check Profit Factor
        if metrics.profit_factor < self.config.target_profit_factor:
            issues.append(f"Profit factor {metrics.profit_factor:.2f} < {self.config.target_profit_factor}")
    ...
    def run_full_validation(
        self,
        output_file: str = "test_report_full_validation.json"
    ) -> ValidationReport:
        """
        ...
        wf_passed = (
            wf_result.mean_return > 0 and
            wf_result.bootstrap_ci_lower > -0.05 and
            wf_result.p_value > 0.5
        )
        ...
        with open(output_file, 'w') as f:
            json.dump({
                ...
                "walk_forward": {
                    ...
                    "bootstrap_ci": [wf_result.bootstrap_ci_lower, wf_result.bootstrap_ci_upper],
...
def main():
    ...
    parser.add_argument('--level', type=int, choices=[1, 2, 3, 4], 
                        help='Run specific validation level (1-4)')
    parser.add_argument('--output', type=str, default='test_report_full_validation.json',
