...
class TestMomentumStrategy:
    ...
    def _create_sample_data(self, num_bars: int = 60) -> pd.DataFrame:
        ...
        dates = pd.date_range(start=datetime.now() - timedelta(days=num_bars), periods=num_bars, freq='1h')
...
class TestMeanReversionStrategy:
    ...
    def _create_sample_data(self, num_bars: int = 60) -> pd.DataFrame:
        ...
        base_price = 50000
        # Mean reverting data
        prices = base_price + 1000 * np.sin(np.linspace(0, 4*np.pi, num_bars)) + np.random.normal(0, 100, num_bars)
