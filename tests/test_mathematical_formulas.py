...
class TestSharpeRatio:
    ...
    def test_sharpe_ratio_positive_returns(self, sample_returns):
        ...
        ann_return = sample_returns.mean() * periods_per_year
        ann_vol = sample_returns.std() * np.sqrt(periods_per_year)
        expected_sharpe = ann_return / ann_vol
        ...
        assert abs(expected_sharpe - actual_sharpe) < 0.001, (
            f"Sharpe ratio mismatch: {expected_sharpe} vs {actual_sharpe}"
        )
...
class TestSortinoRatio:
    ...
    def test_sortino_ratio(self, sample_returns):
        ...
        if len(downside_returns) > 0 and downside_returns.std() > 0:
            ...
            expected_sortino = ann_return / downside_std
        else:
...
class TestRiskReward:
    ...
    def test_risk_reward_ratio(self):
        ...
        trades = pd.DataFrame({"pnl": [100, -50, 150, -30, 200, -40, 80, -20]})
