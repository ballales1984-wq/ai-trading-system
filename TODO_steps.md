...
Scopo: trasformare il progetto in "production-ready" con passi piccoli,
verificabili e con evidenze.
...
- `pytest --maxfail=1 -q` ➜ **FAIL** su `tests/test_bybit.py` per chiamata rete
  esterna (Bybit non raggiungibile nell'ambiente).
- `pytest -q tests/test_app_core_modules.py` ➜ **PASS** (34 passed, warning
  presenti).
...
## 1) Piano operativo per priorita

## P0 — Stabilizzare pipeline test (subito)

### P0.1 Isolare test esterni (Bybit/network)

- [ ] Marcare i test che richiedono rete/API esterne con marker `integration` o
  `external`.
...
- [ ] Allineare `pytest.ini` ai plugin installati oppure installare plugin
  mancanti.
...
- [ ] In `live`, vietare fallback mock silenziosi: ritornare errore typed +
  `data_quality`.

### P1.2 Risk metrics affidabili

- [ ] Evitare fallback sintetico non dichiarato per metriche live.
- [ ] Se storico insufficiente: stato `insufficient_data` (non numero
  artificiale).
...
- [ ] Risk API esplicita sulla qualita/sorgente dati.
...
- [ ] Metriche minime: order failure rate, data freshness, reconciliation
  mismatch.
...
- [ ] Fix `datetime.utcnow()` e deprecazioni piu rumorose.
